...
{
    private const int MainListId = 0;
    ...
    public async Task<List<DialogFilterSnapshot>> GetFiltersAsync(long userId) =>
        (await ReadRowsAsync(userId)).Where(x => x.Id != MainListId).ToList();

    public async Task<List<byte[]>> GetFilterVectorAsync(long userId)
    {
        List<DialogFilterSnapshot> rows = await ReadRowsAsync(userId);
        var values = new List<byte[]>(rows.Count + 1);
        if (rows.All(x => x.Id != MainListId))
        {
            values.Add(MainList());
        }
        foreach (DialogFilterSnapshot row in rows)
        {
            values.Add(row.Filter);
        }
        return values;
    }

    private async Task<List<DialogFilterSnapshot>> ReadRowsAsync(long userId)
    {
        ...
        IReadOnlyList<int> order)
    {
        ...
        int[] positions = order.Contains(MainListId)
            ? order.ToArray()
            : order.Prepend(MainListId).ToArray();
        byte[] mainList = MainList();
        for (int position = 0; position < positions.Length; position++)
        {
            int filterId = positions[position];
            byte[] filter = filterId == MainListId
                ? mainList
                : byId[filterId].Filter;
            using TLDialogFilterState row = DialogFilterState.Builder()
                .UserId(userId).FilterId(filterId).Position(position)
                .Filter(filter).Date(Now()).Build();
            if (!_dialogOrganizationRepository.PutFilter(row))
            {
                return Error(500, "INTERNAL_SERVER_ERROR"u8);
            }
    ...
    private static byte[] MainList()
    {
        using var main = DialogFilterDefault.Builder().Build();
        return main.ToReadOnlySpan().ToArray();
    }

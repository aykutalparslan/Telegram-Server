...
/// <summary>
/// Translates a request's `InputReplyTo` into the stored `MessageReplyHeader`.
/// Sends and forwards must produce the same header for the same input, so this
...
public static class MessageReplyHeaders
{
    ...
    /// <summary>
    /// A forward addressed at a forum topic carries no reply of its own; the
    /// header exists only to name the topic the copy belongs to.
    /// </summary>

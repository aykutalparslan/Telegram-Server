...
public sealed class DeleteSavedHistoryHandler : MessagesHandlerBase
{
    ...
    [TLFunction(Constructors.baseLayer_DeleteSavedHistory)]
    public async Task<TLAffectedHistory> Handle(long authKeyId, TLBytes q)
        {
            ...
            var request = (DeleteSavedHistory)q;
            ...
            var (pts, count) = await DeleteConversation(userId, TLPeer.PeerType.PeerUser,
                partnerId, maxId, minDate, maxDate, userCtx);

            _log.Debug($"🧹 DeleteSavedHistory user:{userId} peer:{partnerId} maxId:{maxId} count:{count} pts:{pts}");
            return AffectedHistory.Builder().Pts(pts).PtsCount(count).Offset(0).Build();
        }
}

...
public sealed class SendPipeline
{
    ...
    public SendPipeline(IUnitOfWork unitOfWork, IChannelMessagesRepository channelMessagesRepository, IChatParticipantsRepository chatParticipantsRepository, IForumTopicsRepository forumTopicsRepository, IMessageInteractionsRepository messageInteractionsRepository, IMessageReactionsRepository messageReactionsRepository, IMessageRepository messageRepository, IUserRepository userRepository, MessageStore messages,
        ...
        byte[]? media = null, long groupedId = 0)
    {
        ...
        await PublishToBoxesAsync(RecipientBox(userId, peerId), async () =>
        {
            if (peerId != userId)
            {
                pending = await PutIncomingAsync(peerId, outgoingMessage, from, to,
                    logicalId, ttlPeriod);
            }
        ...
        byte[]? chatBytes = null)
    {
        ...
        await PublishToBoxesAsync(pending.Select(d => d.RecipientId).ToArray(), async () =>
        {
            await _unitOfWork.SaveAsync();
        ...
        byte[]? media = null, long groupedId = 0)
    {
        ...
        try
        {
            ...
            if (broadcast)
            {
                RecordInitialBroadcastView(channelId, messageId, userId,
    ...
    private void RecordInitialBroadcastView(long channelId, int messageId,
        long userId, int date)
    {
        using TLMessageViewReceipt receipt = MessageViewReceipt.Builder()
            .BoxType(MessageIdentityBox.Channel)
            .BoxId(channelId)
            .MessageId(messageId)
            .UserId(userId)
            .Date(date)
        ...
        using TLMessageInteractionInfo counters = MessageInteractionInfo.Builder()
            .BoxType(MessageIdentityBox.Channel)
            .BoxId(channelId)
            .MessageId(messageId)
            .Views(1)
            .Forwards(0)
            .Build();
        _messageInteractionsRepository.PutInteractionInfo(counters);
    }
    ...
    private static IReadOnlyCollection<long> RecipientBox(long userId, long peerId) =>
        peerId == userId ? Array.Empty<long>() : new[] { peerId };

    private async Task PublishToBoxesAsync(IReadOnlyCollection<long> recipientIds,
        Func<Task> commitAndDeliver)
    {
        var boxes = new List<IUpdatesContext>(recipientIds.Count);
        try
        {
            foreach (long recipientId in recipientIds)
            {
                IUpdatesContext box = _updatesContextFactory.GetUpdatesContext(null,
                    recipientId);
                await box.BeginPtsPublication();
                boxes.Add(box);
            }
            await commitAndDeliver();
        }
        finally
        {
            foreach (IUpdatesContext box in boxes)
            {
                try
                {
                    await box.CompletePtsPublication();
                }
                catch (Exception error)
                {
                    _log.Warning($"pts publication was not released: {error.Message}");
                }
            }
        }
    }

...
/// <summary>
/// Selects the caller's unread mentions in one dialog. The two boxes answer the
/// same question from different state: a common-box copy is per owner and carries
/// the `mentioned` flag itself, while a channel post is shared and can only be
/// matched against the caller by its own mention entities, with the read side
/// coming from the per-viewer content-read rows.
/// </summary>
public sealed class MentionScope
{
    ...
    /// <summary>
    /// The caller's own unread mentions, newest first. `media_unread` is not part
    /// of the test: `messages.readMentions` clears only `mentioned`, so a voice
    /// message whose mention was caught up on stays unread content.
    /// </summary>

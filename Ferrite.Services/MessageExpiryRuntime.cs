...
/// <summary>
/// The due coordinator for history TTL: it deletes every stored copy whose
/// `ttl_period` has elapsed and reports each deletion as an ordinary delete update
/// in that copy's own box. Unlike the scheduled queue there is no claim, because
/// deleting is idempotent: a pass that dies halfway leaves rows whose messages are
/// already gone, and the next pass retires those without telling anyone twice.
/// Startup reconciliation is therefore just the first pass.
/// </summary>
public sealed class MessageExpiryRuntime
{
    ...
    /// <summary>
    /// Deletes every copy whose expiry has arrived. Rows are grouped by box so one
    /// owner's batch advances that box's pts once and arrives as a single
    /// `updateDeleteMessages`, exactly as an explicit multi-id delete does.
    /// </summary>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            // Committed before anyone is told, so a failed write never leaves a
            // client believing a message it still holds is gone.
        ...
        long ownerId, IEnumerable<MessageExpiryStore.ExpirySnapshot> entries)
    {
        ...
        {
            // A copy the owner already deleted leaves its row behind; retiring it
            // silently is correct, because reporting a delete for a message the
            // client no longer holds would advance pts for nothing.
    ...
    {
        // No member is the actor of an expiry, so every member is told; the
        // poster learns their own post is gone from the same update.

...
public sealed class DialogBuilder
{
    ...
    /// <summary>
    /// The caller's own copies of one conversation, newest first. Handlers that
    /// answer a filtered slice of history (unread mentions, recent live
    /// locations) narrow this list and hand it back to
    /// <see cref="BuildCommonMessagesAsync"/> so pagination and peer hydration
    /// stay identical to plain history.
    /// </summary>
    ...
    /// <summary>
    /// The caller's whole message box, every conversation at once, each row still
    /// naming the conversation it belongs to. Global search spans conversations,
    /// so it cannot use the peer-filtered
    /// <see cref="ReadCommonConversationAsync"/>.
    /// </summary>
        ...
        string caller)
    {
        ...
        return BuildMessages(selected, relatedUserIds, relatedChatBytes);
    }
        ...
        HistoryQuery query, int totalCount, string caller, bool reportOffset)
    {
        ...
        return BuildChannelMessages(channelPts, totalCount, selected, relatedUserIds,
            relatedChatBytes, reportOffset ? startIndex : null);
    }

    /// <summary>
    /// One page of a peer-scoped search over the caller's own box. The result is
    /// `messages.messagesSlice` rather than `messages.messages`, because a search
    /// answer must carry the FULL match count and this page's position in it;
    /// plain history has neither.
    /// </summary>
        ...
        HistoryQuery query, string caller)
    {
        ...
        return BuildMessagesSlice(matched.Count, startIndex, null, selected,
            relatedUserIds, relatedChatBytes);
    }

    /// <summary>
    /// One page of a channel-scoped search. A channel request must answer with the
    /// channel constructor: pinned TDLib logs an error and repairs the dialog when
    /// it receives a non-channel one (`MessagesManager.cpp:1166-1176`).
    /// </summary>
    ...
    /// <summary>
    /// The users and chats a set of selected rows needs in order to be rendered,
    /// for results that are not a `messages.Messages` at all -- the search
    /// calendar builds its own vectors. Both collections are heap types, because a
    /// <see cref="Vector"/> is a ref struct and cannot cross the await this read
    /// performs.
    /// </summary>
    ...
    /// <summary>
    /// One page of a search that spans conversations. Peers are hydrated purely
    /// from the selected rows, because no single conversation owns the result.
    /// </summary>
        ...
        IReadOnlyList<byte[]> selected, int totalCount, int? nextRate, string caller)
    {
        ...
        return BuildMessagesSlice(totalCount, null, nextRate, selected, relatedUserIds,
            relatedChatBytes);
    }

    /// <summary>
    /// One page of a public-post search. It is a global slice plus the search
    /// flood row, which pinned TDLib requires: `SearchPublicPostsQuery` fails the
    /// whole request when the slice does not carry one
    /// (`MessageQueryManager.cpp:578-584`). The row arrives as a managed array
    /// because it is built before this method's awaits and a pooled value cannot
    /// cross one.
    /// </summary>
    ...
    /// <summary>
    /// Builds a `messages.Messages` result for an already selected, already ordered
    /// list of rows. A scheduled queue is neither anchored nor paginated, so it
    /// bypasses <see cref="SelectHistory"/> while keeping the same peer hydration.
    /// The channel form is not cosmetic: pinned TDLib logs an error and repairs the
    /// dialog when a channel request answers with the non-channel constructor
    /// (`MessagesManager.cpp:1166-1176`).
    /// </summary>
        ...
        TLPeer.PeerType peerType, long peerId, IReadOnlyList<byte[]> selected)
    {
        ...
        {
            return BuildMessages(rows, relatedUserIds, relatedChatBytes);
        }

        var channelBox = new ChannelMessageBox(_counterFactory, peerId);
        int channelPts = await channelBox.Pts();
        return BuildChannelMessages(channelPts, rows.Count, rows, relatedUserIds,
            relatedChatBytes);
    }
        ...
        DialogQuery query)
    {
        ...
        _log.Debug($"🗂 GetDialogs user:{userId} total:{orderedPeers.Count} " +
        ...
        Dictionary<DialogPeerKey, DialogDraftSnapshot> drafts)
    {
        ...
        return Dialogs.Builder().DialogsProperty(dialogs).Messages(messages).Chats(chats)
            .Users(users).Build();
    }
    ...
    private TLMessages BuildMessages(List<byte[]> messageBytes,
        IEnumerable<long> userIds, IReadOnlyCollection<byte[]> chatBytes)
    {
        ...
        var chats = BuildVector(chatBytes);
        return Messages.Builder().MessagesProperty(messages).Chats(chats).Users(users)
            .Build();
    }

    private TLMessages BuildChannelMessages(int pts, int totalCount,
    ...
    /// <summary>
    /// A `messages.messagesSlice`: the page plus the total match count. Only a
    /// search answers with one, so <paramref name="offsetIdOffset"/> and
    /// <paramref name="nextRate"/> are set exactly when the search that produced
    /// it can state them -- a peer-scoped page knows its index, a global page
    /// knows the date its next page resumes from. <paramref name="searchFlood"/>
    /// belongs to public-post search alone.
    /// </summary>
    private TLMessages BuildMessagesSlice(int totalCount, int? offsetIdOffset,
    ...
    // Resolved before the dialog vector is built: `Vector` is a ref struct and the
    // per-conversation timer is a repository read.
...
/// <summary>
/// One stored row of a message box together with the conversation it belongs to.
/// A conversation-scoped read drops the peer because every row shares it; a
/// box-wide read cannot.
/// </summary>

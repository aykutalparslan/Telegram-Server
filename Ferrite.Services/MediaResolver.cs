...
// Shared media-message helper. Resolving an InputMedia turns an uploaded or reused
// photo/document into the wire messageMedia* bytes stored on the message row, reusing
// the profile-photo pipeline for photos and IUploadService.RegisterDocument for
// documents. Uploaded photos are finalized+processed (thumbnail ladder + canonical
// photo# row); reused ids are re-validated against the stores (MEDIA_INVALID on
// mismatch). External-URL media is rejected (the server never fetches remote URLs).
public static class MediaResolver
{
    ...
    // Await-safe snapshot of an input geo point. A live location that is being
    // stopped carries inputGeoPointEmpty, because the client is no longer
    // reporting a position; the caller substitutes the last known point.
    private readonly record struct GeoSnapshot(bool Present, double Lat,
        double Longitude, int AccuracyRadius);

    // Resolves the serialized InputMedia bytes into the wire messageMedia* bytes or a
    // typed error. Ref-struct views are read synchronously into heap values before any
    // await; all storage/async work happens afterward.
        ...
        IDocumentsRepository documentsRepository)
    {
        ...
        {
            // A stopped live location, and one sent without a validity
            // period, are both ordinary static points: nothing will follow.
            ...
            if (live.Stopped)
            {
                // Stopping keeps the row live-shaped so the client still
                // renders it as a finished live location; the elapsed period
                // is applied by the caller, which knows the message date.
    ...
    public static MediaCategory GetCategory(Span<byte> inputMediaBytes)
    {
        ...
        // A poll is classified here so the send path can check the send_polls
        // right, but it is never resolved by ResolveAsync: a poll needs an
        // allocated id and, once sent, a durable definition keyed by the
        // resulting message identity. PollStore owns both.
    ...
    /// <summary>
    /// Applies the elapsed lifetime to a live location that the sender stopped.
    /// A stop carries no position, so the row keeps the last one it already had,
    /// and its period is shortened to the time already passed. Expiry and an
    /// explicit stop then look identical to every reader, which is what lets one
    /// `date + period > now` rule answer both.
    /// </summary>
    ...
    // Finalizes and processes an uploaded document thumbnail through the photo pipeline,
    // re-keys the generated thumbnail rows under the document id (so
    // inputDocumentFileLocation with a thumb_size resolves), and returns the resulting
    // PhotoSize vector for the document# thumbs field. A thumb that fails to process is
    // dropped (documents keep their bytes regardless of a bad client thumbnail).

...
public sealed class UpdateFanout
{
    ...
    public UpdateFanout(IUnitOfWork unitOfWork, IChannelMessagesRepository channelMessagesRepository, IChatParticipantsRepository chatParticipantsRepository, IChatRepository chatRepository, IContactsRepository contactsRepository, IUserRepository userRepository, IUserStatusRepository userStatusRepository, IUpdatesService updates,
        ...
        int pts)
    {
        TLUpdate update = UpdateNewMessage.Builder()
            ...
            .PtsCount(1)
            .Build();
        await _updates.EnqueueMessageUpdate(recipientId, update, pts);
    }
        ...
        bool pinned, int pts, int ptsCount)
    {
        ...
        return BuildUpdates(userId, new[] { update.AsSpan().ToArray() }, userIds, chatBytes,
            (int)DateTimeOffset.Now.ToUnixTimeSeconds(), seq: 0);
    }

    public async Task<TLUpdates> CompleteBasicGroupServiceResultAsync(
        long callerUserId, IReadOnlyCollection<long> participantIds,
        ...
        byte[]? sharedUpdateBytes, int date)
    {
        ...
        return BuildUpdates(callerUserId, resultBytes, participantIds, new[] { chatBytes }, date,
            seq: 0);
    }
    ...
    public TLUpdates BuildUpdates(long viewerUserId, IReadOnlyCollection<byte[]> updateBytes,
        ...
        int date, int seq)
    {
        ...
        AppendUsers(viewerUserId, ref users, userIds);
    ...
    public TLUser WithStatus(long viewerUserId, TLUser user) => _userSerializer.WithStatus(viewerUserId, user);

    public void AppendUsers(long viewerUserId, ref Vector users, IEnumerable<long> userIds)
    {
        ...
        foreach (long userId in userIds)
        {
            ...
            _userSerializer.Append(viewerUserId, ref users, userId);
        }
    }

...
public sealed class SetContactSignUpNotificationHandler : AccountHandlerBase
{
    private readonly IAuthorizationRepository _authorizationRepository;
    private readonly ISignUpNotificationRepository _signUpNotificationRepository;

    public SetContactSignUpNotificationHandler(ISearchEngine search, IUpdatesService updates, IRandomGenerator random,
    ...
    [TLFunction(Constructors.baseLayer_SetContactSignUpNotification)]
    public async ValueTask<TLBool> Handle(long authKeyId, TLBytes q)
        {
            bool silent = ((SetContactSignUpNotification)q).Silent;
            var auth = await _authorizationRepository.GetAuthorizationAsync(authKeyId);
            if(auth == null) return (TLBool)RpcErrorGenerator.GenerateError(400, "AUTH_KEY_INVALID"u8);
            _signUpNotificationRepository.PutSignUpNotification(auth.Value.AsAuthInfo().UserId, silent);

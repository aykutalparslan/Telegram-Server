...
public sealed class ProfileStore
{
    ...
    public ProfileStore(IAccountSettingsRepository profiles,
        IUnitOfWork transactions, IAuthorizationRepository authorizationRepository, IContactsRepository contactsRepository, IUserRepository userRepository, UserSerializer userSerializer, IUpdatesService updates,
    ...
    public async Task<TLBytes> GetBirthdaysAsync(long userId)
    {
        ...
        try
        {
            foreach (TLContact contact in contacts)
            {
                ...
                using TLUser hydrated = await HydrateUserAsync(userId, contactId,
                    source.Value);
                TLUser prepared = await _userSerializer.WithStatusAsync(userId, hydrated);
        ...
        string rawToken)
    {
        ...
        using TLUser hydrated = await HydrateUserAsync(userId, targetId,
            target.Value);
        using TLUser prepared = await _userSerializer.WithStatusAsync(userId, hydrated);
        byte[] bytes = prepared.AsSpan().ToArray();
        return new TLBytes(bytes, 0, bytes.Length);
    }

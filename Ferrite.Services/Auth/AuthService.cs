...
public class AuthService : IAuthService
{
    ...
    public async ValueTask<TLAuthorization> SignUp(long authKeyId, TLBytes q)
    {
        ...
        {
            if (await RepeatedSignUp(authKeyId, signUpParameters.PhoneNumber) is { } repeated)
            {
                return repeated;
            }
            _log.Debug($"*** Sign Up has no pending login attempt for authKey " +
                       $"{authKeyId} phone {signUpParameters.PhoneNumber} ***");
            return (TLAuthorization)RpcErrorGenerator.GenerateError(400,
        ...
        using (pending)
        {
            ...
            if (!matches)
            {
                _log.Debug($"*** Sign Up attempt mismatch: key {view.AuthKeyId}/{authKeyId} " +
                           $"expires {view.ExpiresAt}/{UnixNow()} " +
                           $"phone {Encoding.UTF8.GetString(view.Phone)}/{signUpParameters.PhoneNumber} " +
                           $"hash {Encoding.UTF8.GetString(view.PhoneCodeHash)}/{signUpParameters.PhoneCodeHash} ***");
            }
        }
        if (!matches)
        {
    ...
    private async ValueTask<TLAuthorization?> RepeatedSignUp(long authKeyId,
        string phoneNumber)
    {
        ...
        using (existing)
        {
            var view = existing.AsAuthInfo();
            registered = view.LoggedIn && view.Phone.SequenceEqual(
                Encoding.UTF8.GetBytes(phoneNumber));
        }

        return registered ? await _authorizationCompletion.CompleteAsync(authKeyId) : null;
    }

...
public sealed partial class StickerStore
{
        ...
        IReadOnlyList<StickerItemInput> items, (long Id, long AccessHash)? thumb)
    {
        ...
        {
            return Error(!IsShortNameValid(shortName)
                ? "SHORT_NAME_INVALID" : "STICKERSET_INVALID");
        }
        using (TLStickerSetState? occupied =
               await _repository.GetSetByShortNameAsync(shortName))
        {
            if (occupied is not null) return Error("SHORT_NAME_OCCUPIED");
        }
        ...
        {
            TLDocument? source = await GetDocumentAsync(item.DocumentId,
                item.AccessHash);
            if (source is null)
            {
        ...
        try
        {
            ...
            if (thumb.HasValue)
            {
                using TLDocument? thumbDocument = await GetDocumentAsync(
                    thumb.Value.Id, thumb.Value.AccessHash);
                if (thumbDocument is null) return Error("STICKER_ID_INVALID");
        ...
        long? accessHash, string? shortName, StickerItemInput item)
    {
        ...
        if (row is null) return Error("STICKERSET_OWNER_ANONYMOUS");
        StickerSetKind kind = Kind(row.Value.AsStickerSetState().Get_SetView()
            .AsStickerSet());
        if (!IsItemValid(kind, item)) return Error("STICKER_INVALID");
        using TLDocument? source = await GetDocumentAsync(item.DocumentId,
            item.AccessHash);
        if (source is null) return Error("STICKER_ID_INVALID");
        ...
        using TLDocument decorated = Decorate(source.Value.AsDocument(),
            view.SetId, oldSet.AccessHash, Kind(oldSet), oldSet.TextColor, item);
        Vector documents = CopyObjectVector(view.Documents);
        documents.AppendTLObject(decorated.AsSpan());
        Vector packs = CopyObjectVector(view.Packs);
        Vector keywords = CopyObjectVector(view.Keywords);
        ...
        long documentAccessHash, StickerItemInput replacement)
    {
        ...
        if (row is null) return Error("STICKER_ID_INVALID");
        using TLDocument? source = await GetDocumentAsync(replacement.DocumentId,
            replacement.AccessHash);
        if (source is null) return Error("STICKER_ID_INVALID");
        var view = row.Value.AsStickerSetState();
        StickerSet oldSet = view.Get_SetView().AsStickerSet();
        if (!IsItemValid(Kind(oldSet), replacement) ||
            replacement.DocumentId != documentId &&
            ContainsDocument(view.Documents, replacement.DocumentId))
            return Error("STICKER_INVALID");
        using TLDocument decorated = Decorate(source.Value.AsDocument(),
            view.SetId, oldSet.AccessHash, Kind(oldSet), oldSet.TextColor,
            replacement);
        ...
        long accessHash, int position)
    {
        ...
        TLBytes result = BuildMutation(view, oldSet,
            CopyObjectVector(view.Packs), CopyObjectVector(view.Keywords),
        ...
        using (owned)
        {
            ...
            var item = new StickerItemInput(documentId, accessHash,
                ...
                keywordsText is null ? ReadKeywords(view.Keywords, documentId)
                    : SplitKeywords(keywordsText));
            if (!IsItemValid(Kind(oldSet), item)) return Error("STICKER_INVALID");
            using TLDocument decorated = Decorate(owned.Value.AsDocument(),
                view.SetId, oldSet.AccessHash, Kind(oldSet), oldSet.TextColor,
                item);
            Vector documents = ReplaceDocument(view.Documents, documentId,
                decorated.AsSpan());
            Vector packs = emoji is null ? CopyObjectVector(view.Packs)
                : RemovePackMetadata(view.Packs, documentId);
            Vector keywords = keywordsText is null
                ? CopyObjectVector(view.Keywords)
        ...
        long? accessHash, string? shortName, long thumbId, long? thumbAccessHash)
    {
        ...
        if (!contained)
        {
            if (!thumbAccessHash.HasValue) return Error("STICKER_ID_INVALID");
            using TLDocument? thumb = await GetDocumentAsync(thumbId,
                thumbAccessHash.Value);
            if (thumb is null) return Error("STICKER_ID_INVALID");
        }
        var view = row.Value.AsStickerSetState();
        StickerSet oldSet = view.Get_SetView().AsStickerSet();
        Vector packs = CopyObjectVector(view.Packs);
    ...
    {
        if (title.Length is < 1 or > 64) return Error("STICKERSET_INVALID");
        using TLStickerSetState? row = await ResolveOwnedSetAsync(userId, setId,
            accessHash, shortName);
        if (row is null) return Error("STICKERSET_OWNER_ANONYMOUS");
        var view = row.Value.AsStickerSetState();
        ...
        long? accessHash, string? shortName)
    {
        ...
        try
        {
            foreach (TLStickerAccountState account in accounts)
            {
                ...
                using TLStickerAccountState updated = StickerAccountState.Builder()
                    .UserId(state.UserId).Revision(state.Revision + 1)
                    .Installed(ToLongVector(Remove(state.Installed.ToArray(), id)))
                    .Archived(ToLongVector(Remove(state.Archived.ToArray(), id)))
                    .SavedGifs(ToLongVector(state.SavedGifs.ToArray().Where(
                        value => !documentIds.Contains(value))))
                    .Recent(ToLongVector(state.Recent.ToArray().Where(
                        value => !documentIds.Contains(value))))
                    .RecentDates(FilterDates(state.Recent.ToArray(),
                        state.RecentDates.ToArray(), documentIds))
                    .AttachedRecent(ToLongVector(state.AttachedRecent.ToArray()
                        .Where(value => !documentIds.Contains(value))))
                    .AttachedRecentDates(FilterDates(
                        state.AttachedRecent.ToArray(),
                        state.AttachedRecentDates.ToArray(), documentIds))
                    .Faved(ToLongVector(state.Faved.ToArray().Where(
                        value => !documentIds.Contains(value))))
                    .FeaturedRead(ToLongVector(Remove(
                        state.FeaturedRead.ToArray(), id))).Build();
                if (!_repository.PutAccountState(updated)) return StorageError();
            }
        }
        finally
        {
        ...
        try
        {
            foreach (TLChannelStickerState channel in channels)
            {
                ...
                else
                {
                    ...
                    if (!_repository.PutChannelState(updated)) return StorageError();
                }
            }
        }
        finally
        ...
        if (!await _repository.DeleteSetAsync(id) ||
            !await _unitOfWork.SaveAsync()) return StorageError();
        return True();
    }
    ...
    {
        TLStickerSetState? row = await ResolveSetAsync(setId, accessHash,
            shortName);
    ...
    private static Vector RemovePackMetadata(Vector source, long documentId)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            using StickerPack updated = StickerPack.Builder()
                .Emoticon(pack.Emoticon).Documents(ToLongVector(ids)).Build();
    ...
    private static VectorOfInt FilterDates(long[] ids, int[] dates,
        HashSet<long> deleted)
    {
        var result = new VectorOfInt();
        for (int i = 0; i < ids.Length; i++)
            if (!deleted.Contains(ids[i])) result.Append(i < dates.Length ? dates[i] : 0);
        return result;
    }

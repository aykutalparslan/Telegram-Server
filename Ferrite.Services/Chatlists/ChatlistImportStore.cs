...
public sealed class ChatlistImportStore
{
    ...
    public ChatlistImportStore(IUnitOfWork unitOfWork,
        IChatParticipantsRepository chatParticipantsRepository,
        IChatRepository chatRepository,
        IDialogOrganizationRepository dialogOrganizationRepository,
        ChatRowStore chatRows, UpdateFanout fanout, IUpdatesService updates,
    ...
    public async Task<TLBytes> GetLeaveSuggestionsAsync(long userId, int filterId)
    {
        ...
        Vector peers = ChatlistPeerCodec.BuildPeerVector(filter.Value.Peers);
    ...
    private async Task<InviteImportSnapshot?> GetActiveInviteAsync(string slug)
    {
        ...
        if (value.Revoked || !ChatlistPeerCodec.TryReadPeers(value.Peers,
        ...
        ShareableFilterSnapshot source, IReadOnlyList<DialogPeerKey> peers)
    {
        ...
        var builder = ChatlistInvite.Builder().Title(title)
            .Peers(ChatlistPeerCodec.BuildPeerVector(peers)).Chats(chats).Users(new Vector());
        ...
        IReadOnlyList<DialogPeerKey> already)
    {
        ...
        ChatlistInviteAlready result = ChatlistInviteAlready.Builder()
            .FilterId(filterId).MissingPeers(ChatlistPeerCodec.BuildPeerVector(missing))
            .AlreadyPeers(ChatlistPeerCodec.BuildPeerVector(already))
        ...
        IReadOnlyList<DialogPeerKey> missing)
    {
        ...
        ChatlistUpdates result = ChatlistUpdates.Builder()
            .MissingPeers(ChatlistPeerCodec.BuildPeerVector(missing))
            .Chats(BuildObjectVector(chatRows)).Users(new Vector()).Build();
        return result.TLBytes!.Value;
    }

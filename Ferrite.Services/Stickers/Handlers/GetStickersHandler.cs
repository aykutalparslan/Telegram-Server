...
{
    public GetStickersHandler(IUnitOfWork unitOfWork, IAuthorizationRepository authorizationRepository, StickerStore store)
    ...
    public async Task<TLBytes> Handle(long authKeyId, TLBytes q)
    {
        ...
        return await GetUserIdAsync(authKeyId) is not null
            ? await Store.GetStickersAsync(emoticon, hash) : AuthError();
    }
}
